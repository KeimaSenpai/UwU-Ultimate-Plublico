...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/start' in msgText:
            ...
            jdb.save()
            statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
            bot.editMessageText(message,"✅Cursad Jovenclud configuration loaded\n https://cursad.jovenclub.cu/")
        elif '/eva' in msgText:
            getUser = user_info
        ...
        elif '/ucmc_cfg' in msgText:
            getUser = user_info
            getUser['moodle_host'] = "http://www.ucmc.cfg.sld.cu/"
            getUser['uploadtype'] =  "calendar"
            getUser['moodle_user'] = "---"
            getUser['moodle_password'] = "---"
            getUser['moodle_repo_id'] = 11
            getUser['zips'] = 9
            jdb.save_data_user(username,getUser)
            jdb.save()
            statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
...
def main():
    bot_token = '5454547683:AAHKfDHdXrScrqrhdqk-j_fkCKVQH1jWsgs'
